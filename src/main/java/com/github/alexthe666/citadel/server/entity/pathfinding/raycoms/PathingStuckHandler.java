...
public class PathingStuckHandler implements IStuckHandler {
    ...
    public void completeStuckAction(final AbstractAdvancedPathNavigate navigator) {
        ...
        if (takeDamageOnCompleteStuck) {
            entity.hurtOrSimulate(new DamageSource(entity.level().damageSources().inWall().typeHolder(), entity), entity.getMaxHealth() * damagePct);
        }
    ...
    public static Direction getFacing(final BlockPos pos, final BlockPos neighbor) {
        ...
        return Direction.getApproximateNearest(vector.getX(), vector.getY(), -vector.getZ());
    }
