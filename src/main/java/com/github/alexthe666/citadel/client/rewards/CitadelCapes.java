...
public class CitadelCapes {
    ...
    public static Cape getCurrentCape(Player player){
        ...
        if(com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getBoolean(tag, "CitadelCapeDisabled")){
            return null;
        }
        if(com.github.alexmodguy.alexscaves.server.misc.NbtCompat.contains(tag, "CitadelCapeType")){
            if(com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getString(tag, "CitadelCapeType").isEmpty()){
                return getFirstApplicable(player);
            }else{
                return CitadelCapes.getById(com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getString(tag, "CitadelCapeType"));
            }
        }else{
            return null;
        }
