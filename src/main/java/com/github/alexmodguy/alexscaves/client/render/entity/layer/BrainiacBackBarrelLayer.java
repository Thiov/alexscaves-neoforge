...
public class BrainiacBackBarrelLayer extends RenderLayer121X<BrainiacEntity, BrainiacModel> {
    ...
    public void render(PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn, BrainiacEntity brainiac, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        if (brainiac.hasBarrel()) {
            ...
            com.github.alexmodguy.alexscaves.client.render.compat.BlockRenderCompat.renderSingleBlock(ACBlockRegistry.WASTE_DRUM.get().defaultBlockState(), matrixStackIn, bufferIn, packedLightIn, OverlayTexture.NO_OVERLAY);
