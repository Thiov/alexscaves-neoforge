...
public class AmbersolBlockRenderer<T extends AmbersolBlockEntity> implements com.github.alexmodguy.alexscaves.client.render.blockentity.compat.BlockEntityRenderer121X<T> {
    ...
    private static void renderAt(AmbersolBlockEntity ambersol, float partialTicks, PoseStack matrixStackIn, MultiBufferSource bufferIn) {
        ...
        if (scale > 0.0F) {
            Quaternionf camera = Minecraft.getInstance().getEntityRenderDispatcher().camera.rotation();
            ...
            PoseStack.Pose posestack$pose = matrixStackIn.last();
            Matrix4f matrix4f = posestack$pose.pose();
            VertexConsumer lines = bufferIn.getBuffer(net.minecraft.client.renderer.rendertype.RenderTypes.lines());
            matrixStackIn.popPose();
        }
    }
