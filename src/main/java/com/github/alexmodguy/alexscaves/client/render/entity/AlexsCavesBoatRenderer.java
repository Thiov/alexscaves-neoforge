...
public class AlexsCavesBoatRenderer<T extends Boat & AlexsCavesBoat> extends EntityRenderer121X<T> {

    private final HashMap<AlexsCavesBoat.Type, Identifier> textureMap = new HashMap<>();
    private final HashMap<AlexsCavesBoat.Type, ACBoatModel> modelMap = new HashMap<>();

    private static final Identifier CHEST_TEXTURE = Identifier.fromNamespaceAndPath(AlexsCaves.MODID, "textures/entity/boat/chest.png");
    ...
    public AlexsCavesBoatRenderer(EntityRendererProvider.Context context, boolean isChest) {
        ...
        for (AlexsCavesBoat.Type type : AlexsCavesBoat.Type.values()) {
            textureMap.put(type, Identifier.fromNamespaceAndPath(AlexsCaves.MODID, "textures/entity/boat/" + type.getName() + "_boat.png"));
        }
    ...
    public void render(T entity, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int packedLightIn) {
        ...
        if (isChest) {
            ...
            CHEST_MODEL.renderToBuffer(poseStack, bufferIn.getBuffer(net.minecraft.client.renderer.rendertype.RenderTypes.entityCutout(CHEST_TEXTURE)), packedLightIn, OverlayTexture.NO_OVERLAY, -1);
            poseStack.popPose();
        }
        model.setupAnim(entity, partialTicks, 0.0F, -0.1F, 0.0F, 0.0F);
        VertexConsumer vertexconsumer = bufferIn.getBuffer(net.minecraft.client.renderer.rendertype.RenderTypes.entityCutout(getTextureLocation(entity)));
        model.renderToBuffer(poseStack, vertexconsumer, packedLightIn, OverlayTexture.NO_OVERLAY, -1);
        if (!entity.isUnderWater()) {
            VertexConsumer vertexconsumer1 = bufferIn.getBuffer(net.minecraft.client.renderer.rendertype.RenderTypes.waterMask());
