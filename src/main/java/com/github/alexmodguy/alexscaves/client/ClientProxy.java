...
import com.github.alexmodguy.alexscaves.mcshim.ShaderInstance;
import com.github.alexmodguy.alexscaves.mcshim.ItemColor;
...
import com.github.alexmodguy.alexscaves.client.render.ACFluidRegistryClient;
...
import com.github.alexmodguy.alexscaves.mcshim.ItemProperties;
import com.github.alexmodguy.alexscaves.mcshim.ModelResourceLocation;
...
public class ClientProxy extends CommonProxy {
    ...
    /**
     * 26.1.2 client tick (game bus). Wraps the per-tick client logic (formerly the Fabric
     * {@code ClientTickEvents.END_CLIENT_TICK} callback). Registered on {@code NeoForge.EVENT_BUS} in clientInit.
     */
    private void onClientTick(net.neoforged.neoforge.client.event.ClientTickEvent.Post event) {
        onClientTickWork(Minecraft.getInstance());
    }

    private static void onClientTickWork(Minecraft minecraft) {
    ...
    public void clientInit(IEventBus modEventBus) {
        ...
        // acid / purple-soda fluid render layer is driven by the FluidType + RegisterFluidModelsEvent wiring
        // in ACFluidRegistryClient (registered above), so there is nothing to register here.
    }

    private void registerRenderers(EntityRenderersEvent.RegisterRenderers event) {
        event.registerEntityRenderer(ACEntityRegistry.BOAT.get(), (context) -> {
            return new AlexsCavesBoatRenderer(context, false);
        });
        event.registerEntityRenderer(ACEntityRegistry.CHEST_BOAT.get(), (context) -> {
            return new AlexsCavesBoatRenderer(context, true);
        });
        event.registerBlockEntityRenderer(ACBlockEntityRegistry.MAGNET.get(), MagnetBlockRenderer::new);
        event.registerBlockEntityRenderer(ACBlockEntityRegistry.TESLA_BULB.get(), TelsaBulbBlockRenderer::new);
        event.registerBlockEntityRenderer(ACBlockEntityRegistry.HOLOGRAM_PROJECTOR.get(),
                HologramProjectorBlockRenderer::new);
        event.registerBlockEntityRenderer(ACBlockEntityRegistry.QUARRY.get(), QuarryBlockRenderer::new);
        event.registerBlockEntityRenderer(ACBlockEntityRegistry.AMBERSOL.get(), AmbersolBlockRenderer::new);
        event.registerBlockEntityRenderer(ACBlockEntityRegistry.AMBER_MONOLITH.get(), AmberMonolithBlockRenderer::new);
        event.registerBlockEntityRenderer(ACBlockEntityRegistry.NUCLEAR_FURNACE.get(), NuclearFurnaceBlockRenderer::new);
        event.registerBlockEntityRenderer(ACBlockEntityRegistry.SIREN_LIGHT.get(), SirenLightBlockRenderer::new);
        event.registerBlockEntityRenderer(ACBlockEntityRegistry.ABYSSAL_ALTAR.get(), AbyssalAltarBlockRenderer::new);
        event.registerBlockEntityRenderer(ACBlockEntityRegistry.COPPER_VALVE.get(), CopperValveBlockRenderer::new);
        event.registerBlockEntityRenderer(ACBlockEntityRegistry.BEHOLDER.get(), BeholderBlockRenderer::new);
        event.registerBlockEntityRenderer(ACBlockEntityRegistry.GOBTHUMPER.get(), GobthumperBlockRenderer::new);
        event.registerBlockEntityRenderer(ACBlockEntityRegistry.CONVERSION_CRUCIBLE.get(),
                ConversionCrucibleBlockRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.MOVING_METAL_BLOCK.get(), MovingMetalBlockRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.TELETOR.get(), TeletorRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.MAGNETIC_WEAPON.get(), MagneticWeaponRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.MAGNETRON.get(), MagnetronRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.BOUNDROID.get(), BoundroidRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.BOUNDROID_WINCH.get(), BoundroidWinchRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.FERROUSLIME.get(), FerrouslimeRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.NOTOR.get(), NotorRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.QUARRY_SMASHER.get(), QuarrySmasherRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.SEEKING_ARROW.get(), SeekingArrowRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.SUBTERRANODON.get(), SubterranodonRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.VALLUMRAPTOR.get(), VallumraptorRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.GROTTOCERATOPS.get(), GrottoceratopsRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.TRILOCARIS.get(), TrilocarisRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.TREMORSAURUS.get(), TremorsaurusRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.RELICHEIRUS.get(), RelicheirusRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.FALLING_TREE_BLOCK.get(), FallingTreeBlockRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.CRUSHED_BLOCK.get(), CrushedBlockRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.LIMESTONE_SPEAR.get(), LimestoneSpearRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.EXTINCTION_SPEAR.get(), ExtinctionSpearRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.DINOSAUR_SPIRIT.get(), DinosaurSpiritRenderer::new);
        ...
        event.registerEntityRenderer(ACEntityRegistry.NUCLEAR_EXPLOSION.get(), EmptyRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.NUCLEAR_BOMB.get(), NuclearBombRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.NUCLEEPER.get(), NucleeperRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.RADGILL.get(), RadgillRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.BRAINIAC.get(), BrainiacRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.THROWN_WASTE_DRUM.get(), ThrownWasteDrumEntityRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.GAMMAROACH.get(), GammaroachRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.RAYCAT.get(), RaycatRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.CINDER_BRICK.get(), (context) -> {
            return new ThrownItemRenderer<>(context, 1.25F, false);
        });
        event.registerEntityRenderer(ACEntityRegistry.TREMORZILLA.get(), TremorzillaRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.LANTERNFISH.get(), LanternfishRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.SEA_PIG.get(), SeaPigRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.SUBMARINE.get(), SubmarineRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.HULLBREAKER.get(), HullbreakerRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.GOSSAMER_WORM.get(), GossamerWormRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.TRIPODFISH.get(), TripodfishRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.DEEP_ONE.get(), DeepOneRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.INK_BOMB.get(), (context) -> {
            return new ThrownItemRenderer<>(context, 1.25F, false);
        });
        event.registerEntityRenderer(ACEntityRegistry.DEEP_ONE_KNIGHT.get(), DeepOneKnightRenderer::new);
        ...
        event.registerEntityRenderer(ACEntityRegistry.MINE_GUARDIAN.get(), MineGuardianRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.MINE_GUARDIAN_ANCHOR.get(), MineGuardianAnchorRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.DEPTH_CHARGE.get(), (context) -> {
            return new ThrownItemRenderer<>(context, 1.75F, true);
        });
        event.registerEntityRenderer(ACEntityRegistry.FLOATER.get(), FloaterRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.GUANO.get(), (context) -> {
            return new ThrownItemRenderer<>(context, 1.25F, false);
        });
        event.registerEntityRenderer(ACEntityRegistry.FALLING_GUANO.get(), FallingBlockRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.GLOOMOTH.get(), GloomothRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.UNDERZEALOT.get(), UnderzealotRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.WATCHER.get(), WatcherRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.CORRODENT.get(), CorrodentRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.VESPER.get(), VesperRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.FORSAKEN.get(), ForsakenRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.BEHOLDER_EYE.get(), EmptyRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.DESOLATE_DAGGER.get(), DesolateDaggerRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.BURROWING_ARROW.get(), BurrowingArrowRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.DARK_ARROW.get(), DarkArrowRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.SWEETISH_FISH.get(), SweetishFishRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.CANIAC.get(), CaniacRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.GUMBEEPER.get(), GumbeeperRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.GUMBALL.get(), GumballRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.CANDICORN.get(), CandicornRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.GUM_WORM.get(), GumWormRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.GUM_WORM_SEGMENT.get(), GumWormSegmentRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.CARAMEL_CUBE.get(), CaramelCubeRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.MELTED_CARAMEL.get(), MeltedCaramelRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.GUMMY_BEAR.get(), GummyBearRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.LICOWITCH.get(), LicowitchRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.SPINNING_PEPPERMINT.get(), SpinningPeppermintRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.SUGAR_STAFF_HEX.get(), SugarStaffHexRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.GINGERBREAD_MAN.get(), GingerbreadManRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.FALLING_FROSTMINT.get(), FallingBlockRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.CANDY_CANE_HOOK.get(), CandyCaneHookRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.SODA_BOTTLE_ROCKET.get(), (render) -> {
            return new ThrownItemRenderer<>(render, 1.25F, true);
        });
        event.registerEntityRenderer(ACEntityRegistry.FROSTMINT_SPEAR.get(), FrostmintSpearRenderer::new);
        event.registerEntityRenderer(ACEntityRegistry.THROWN_ICE_CREAM_SCOOP.get(), (context) -> {
            return new ThrownItemRenderer<>(context, 1.25F, false);
        });
    ...
    /**
     * Registers AC's custom client-item / armor extensions (26.1.2 {@link RegisterClientExtensionsEvent},
     * mod bus). The ISTER extension is largely inert (in-hand 3D item rendering is now the data-driven
     * {@code alexscaves:bewlr} item model), but the ARMOR extension's {@code getHumanoidArmorModel} hook IS
     * invoked by {@code EquipmentLayerRenderer.renderLayers} in this build, so registering it is what makes
     * AC's custom 3D armor models render.
     */
    private void registerClientExtensions(RegisterClientExtensionsEvent event) {
        ...
        event.registerItem(ister,
                ...
                ACItemRegistry.EXTINCTION_SPEAR.get(), ACItemRegistry.FROSTMINT_SPEAR.get(),
                ACItemRegistry.SEA_STAFF.get(), ACItemRegistry.ORTHOLANCE.get(),
                ACItemRegistry.GALENA_GAUNTLET.get(), ACItemRegistry.RESISTOR_SHIELD.get(),
    ...
    /**
     * Registers the Cloak of Darkness charge meter HUD just above the hotbar (26.1.2
     * {@link RegisterGuiLayersEvent}, mod bus). Replaces the Fabric HudElementRegistry path.
     */
    ...
    public void onItemColors(RegisterColorHandlersEvent.ItemTintSources event) {
        // 26.1.2: the legacy RegisterColorHandlersEvent.Item + event.register(ItemColor, items...) API was
    ...
    public void onBlockColors(RegisterColorHandlersEvent.BlockTintSources event) {
        // 26.1.2: block tint handlers are registered through the BlockColors instance with a List of the new
        // BlockTintSource interface (color(BlockState) / colorInWorld(state, level, pos)).
        BlockTintSource frostingTint = new BlockTintSource() {
            @Override
            ...
            public int colorInWorld(net.minecraft.world.level.block.state.BlockState state,
                                    ...
                                    BlockPos pos) {
                return FrostedChocolateBlock.calculateFrostingColor(pos);
            }
        };
        event.register(java.util.List.of(frostingTint), ACBlockRegistry.BLOCK_OF_FROSTED_CHOCOLATE.get());
        event.register(java.util.List.of(frostingTint), ACBlockRegistry.BLOCK_OF_FROSTING.get());
    }
    ...
    private void bakeModels(final ModelEvent.ModifyBakingResult e) {
        // 26.1.2: ModelEvent.ModifyBakingResult no longer exposes a ModelResourceLocation->BakedModel map
        // (getModels() is gone). getBakingResult() returns ModelBakery.BakingResult with
        // blockStateModels() (Map<BlockState, BlockStateModel>) and itemStackModels() (Map<Identifier,
        // ItemModel>) — there is no BakedModel to wrap, and the whole BakedModel pipeline was replaced by
        // BlockStateModel. The emissive/fullbright block-model override (BakedModelShadeLayerFullbright)
        // therefore has no hook here and degrades to a no-op.
        // TODO 26.1.2: re-implement the emissiveBlockModels config option against the BlockStateModel
        // pipeline (e.g. wrap the BlockStateModel/quad emitter for the FULLBRIGHTS ids) if wanted.
        if (AlexsCaves.CLIENT_CONFIG.emissiveBlockModels.get()) {
            AlexsCaves.LOGGER.warn("emissiveBlockModels is enabled but is not yet supported on NeoForge 26.1.2 (BakedModel pipeline removed); skipping.");
        }
    }

    // 26.1.2: the legacy core-shader system (RegisterShadersEvent + ShaderInstance + ResourceProvider) was
    // REMOVED from MC/NeoForge — there is no RegisterShadersEvent to listen for, and render setup moved to the
    // RenderPipeline system. AC's custom core shaders (ferrouslime gel, hologram, irradiated, bubbled, sepia,
    // red ghost, purple witch) are no longer registered; ACRenderTypes already maps those render types onto
    // vanilla equivalents / custom RenderPipelines instead, so nothing reads ACInternalShaders anymore.
    // TODO 26.1.2: if pixel-accurate fidelity for those effects is wanted, port them to custom RenderPipelines
    // (see ACRenderTypes' two custom TRIANGLES pipelines for the pattern) rather than core shaders.
