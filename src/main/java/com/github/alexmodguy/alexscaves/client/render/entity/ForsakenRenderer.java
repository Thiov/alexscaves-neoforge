...
public class ForsakenRenderer extends MobRenderer121X<ForsakenEntity, ForsakenModel> {
    private static final Identifier TEXTURE = Identifier.fromNamespaceAndPath(AlexsCaves.MODID, "textures/entity/forsaken.png");
    private static final Identifier TEXTURE_EYES = Identifier.fromNamespaceAndPath(AlexsCaves.MODID, "textures/entity/forsaken_eyes.png");

    private static final Identifier TEXTURE_DARKNESS = Identifier.fromNamespaceAndPath(AlexsCaves.MODID, "textures/entity/forsaken_darkness.png");
    ...
    public Identifier getTextureLocation(ForsakenEntity entity) {
        return TEXTURE;
    }

    class LayerGlow extends RenderLayer121X<ForsakenEntity, ForsakenModel> {
        ...
        public void render(PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn, ForsakenEntity entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
            VertexConsumer darnkessConsumer = bufferIn.getBuffer(net.minecraft.client.renderer.rendertype.RenderTypes.entityTranslucent(TEXTURE_DARKNESS));
            this.getParentModel().renderToBuffer(matrixStackIn, darnkessConsumer, packedLightIn, LivingEntityRenderer121X.getOverlayCoords(entitylivingbaseIn, 0.0F), ColorUtil.packColor(1.0F, 1.0F, 1.0F, entitylivingbaseIn.getDarknessAmount(partialTicks)));
            VertexConsumer eyesConsumer = bufferIn.getBuffer(net.minecraft.client.renderer.rendertype.RenderTypes.eyes(TEXTURE_EYES));
            this.getParentModel().renderToBuffer(matrixStackIn, eyesConsumer, packedLightIn, LivingEntityRenderer121X.getOverlayCoords(entitylivingbaseIn, 0.0F), -1);
        }
    }
}
