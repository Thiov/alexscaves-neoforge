...
public class CaveBookScreen extends Screen {
    ...
    protected Identifier prevEntryJSON = null;
    protected Identifier currentEntryJSON;
    protected Identifier nextEntryJSON;
    ...
    public CaveBookScreen(String openTo) {
        ...
        currentEntryJSON = Identifier.fromNamespaceAndPath(AlexsCaves.MODID, openTo);
        resetEntry();
    }
    ...
    public void resetEntry() {
        ...
        if (this.currentEntry != null && this.currentEntry.getParent() != null && !this.currentEntry.getParent().isEmpty()) {
            this.prevEntryJSON = Identifier.parse(getBookFileDirectory() + this.currentEntry.getParent());
        } else {
    ...
    public void extractRenderState(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float fakePartialTickThatsZeroForSomeReason) {
    ...
    private void renderForPageType(PageRenderer contents, int kind, PoseStack poseStack, int mouseX, int mouseY, float partialTick) {
        ...
        switch (kind) {
            case 0 -> poseStack.translate(-0.1F, -0.1885F, -0.005F);
            case 1 -> poseStack.translate(-0.725F, -0.1885F, -0.005F);
            case 2 -> poseStack.translate(-0.0375F, -0.015F, -0.01F);
            case 3 -> poseStack.translate(-0.7125F, 0.0054F, -0.005F);
        }
    ...
    public boolean mouseClicked(MouseButtonEvent mouseButtonEvent, boolean doubleClick) {
        ...
        int i = this.width / 2;
        float distFromMiddleX = (float) ((mouseX - i) / 220F);
        boolean prev = super.mouseClicked(mouseButtonEvent, doubleClick);
        if (!prev) {
    ...
    @Nullable
    protected BookEntry readBookEntry(Identifier res) {
    ...
    public boolean attemptChangePage(Identifier changePageTo, boolean goingForwards) {
    ...
    public int getEntryVisiblity(String linkTo) {
        Identifier resourceLocation = Identifier.parse(CaveBookScreen.getBookFileDirectory() + linkTo);
