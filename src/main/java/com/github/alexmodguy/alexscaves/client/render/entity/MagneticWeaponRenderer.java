...
public class MagneticWeaponRenderer extends EntityRenderer121X<MagneticWeaponEntity> {
    ...
    public void render(MagneticWeaponEntity entity, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource source, int i) {
        ...
        if (controller instanceof Player player) {
            ...
            if (controller == Minecraft.getInstance().player && Minecraft.getInstance().options.getCameraType().isFirstPerson()) {
                ...
                Vec3 vec3 = this.entityRenderDispatcher.camera.getNearPlane(1.0F).getPointOnPlane((float) leftAmount * 0.525F, -0.7F);
                vec3 = vec3.scale(d7);
        ...
        com.github.alexmodguy.alexscaves.client.render.item.ItemRendererCompat.render(null, itemStack, ItemDisplayContext.THIRD_PERSON_RIGHT_HAND, false, poseStack, source, i, OverlayTexture.NO_OVERLAY, null);
        poseStack.popPose();
    ...
    public Identifier getTextureLocation(MagneticWeaponEntity entity) {
