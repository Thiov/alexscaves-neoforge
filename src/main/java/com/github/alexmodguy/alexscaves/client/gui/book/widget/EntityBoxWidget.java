...
public class EntityBoxWidget extends EntityWidget {
    ...
    public EntityBoxWidget(int displayPage, String entityId, boolean sepia, float boxWidth, float boxHeight, float boxScale,
                           float entityXOffset, float entityYOffset, String borderImage, String entityNBT, int x, int y, float scale) {
    ...
    public void render(PoseStack poseStack, MultiBufferSource.BufferSource bufferSource, float partialTicks, boolean onFlippingPage) {
        if (this.borderTexture == null) {
            this.borderTexture = Identifier.parse(borderImage);
        }
        poseStack.pushPose();
        ...
        renderQuad(poseStack, vertexconsumer, boxWidth - endSub, PIX_WIDTH_CORNER / 2F, -PIX_WIDTH_CORNER / 2F, PIX_WIDTH_CORNER / 2F,
            PIX_WIDTH_CORNER, PIX_WIDTH_CORNER + PIX_WIDTH_LINE, 0, PIX_WIDTH_CORNER);
        ...
        renderQuad(poseStack, vertexconsumer, PIX_WIDTH_CORNER / 2F, -PIX_WIDTH_CORNER / 2F, PIX_WIDTH_CORNER / 2F - 0.1F,
            boxHeight - endSub, 0, PIX_WIDTH_CORNER, PIX_WIDTH_CORNER, PIX_WIDTH_CORNER + PIX_WIDTH_LINE);
        ...
        renderQuad(poseStack, vertexconsumer, boxWidth - endSub, PIX_WIDTH_CORNER / 2F, boxHeight - PIX_WIDTH_CORNER / 2F,
            boxHeight + PIX_WIDTH_CORNER / 2F, PIX_WIDTH_CORNER, PIX_WIDTH_CORNER + PIX_WIDTH_LINE, PIX_WIDTH_LINE + PIX_WIDTH_CORNER,
            PIX_WIDTH_LINE + PIX_WIDTH_CORNER * 2);
        ...
        renderQuad(poseStack, vertexconsumer, boxWidth + PIX_WIDTH_CORNER / 2F, boxWidth - PIX_WIDTH_CORNER / 2F,
            PIX_WIDTH_CORNER / 2F - 0.1F, boxHeight - endSub, PIX_WIDTH_LINE + PIX_WIDTH_CORNER,
            PIX_WIDTH_LINE + PIX_WIDTH_CORNER * 2, PIX_WIDTH_CORNER, PIX_WIDTH_CORNER + PIX_WIDTH_LINE);
    ...
    private void renderQuad(PoseStack poseStack, VertexConsumer vertexconsumer, float x0, float x1, float y0, float y1,
