...
public class FerrouslimeRenderer extends EntityRenderer121X<FerrouslimeEntity> implements CustomBookEntityRenderer {

    private static final Identifier TEXTURE = Identifier.fromNamespaceAndPath(AlexsCaves.MODID, "textures/entity/ferrouslime.png");
    private static final Identifier TEXTURE_EYES = Identifier.fromNamespaceAndPath(AlexsCaves.MODID, "textures/entity/ferrouslime_eyes.png");
    private static final Identifier TEXTURE_GEL = Identifier.fromNamespaceAndPath(AlexsCaves.MODID, "textures/entity/ferrouslime_gel.png");
    ...
    private void renderHead(FerrouslimeEntity entity, float partialTicks, PoseStack poseStack, MultiBufferSource source, int packedLight) {
        ...
        VertexConsumer textureConsumer = source.getBuffer(sepia ? ACRenderTypes.getBookWidget(TEXTURE, true) : net.minecraft.client.renderer.rendertype.RenderTypes.entityCutout(TEXTURE));
        FERROUSLIME_MODEL.renderToBuffer(poseStack, textureConsumer, packedLight, LivingEntityRenderer121X.getOverlayCoords(entity, 0.0F), -1);
        VertexConsumer eyesConsumer = source.getBuffer(sepia ? ACRenderTypes.getBookWidget(TEXTURE_EYES, true) : net.minecraft.client.renderer.rendertype.RenderTypes.eyes(TEXTURE_EYES));
        FERROUSLIME_MODEL.renderToBuffer(poseStack, eyesConsumer, packedLight, LivingEntityRenderer121X.getOverlayCoords(entity, 0.0F), -1);
        poseStack.popPose();
    }
    ...
    private void spikeCubeFace(FerrouslimeEntity entity, Matrix4f matrix4f, Matrix3f matrix3f, VertexConsumer vertexConsumer, float offset, float length, float width, int packedLightIn) {
        int overlayCoords = LivingEntityRenderer121X.getOverlayCoords(entity, 0.0F);
    ...
    public Identifier getTextureLocation(FerrouslimeEntity entity) {
        return TEXTURE;
    }
