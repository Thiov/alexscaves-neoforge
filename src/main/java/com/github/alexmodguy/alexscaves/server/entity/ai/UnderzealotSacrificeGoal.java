...
public class UnderzealotSacrificeGoal extends Goal {
    ...
    private Vec3 groundOf(Vec3 in) {
        ...
        while (!entity.level().isEmptyBlock(blockPos) && blockPos.getY() < entity.level().getMaxY()) {
            blockPos.move(0, 1, 0);
        }
        while (entity.level().isEmptyBlock(blockPos.below()) && blockPos.getY() > entity.level().getMinY()) {
            blockPos.move(0, -1, 0);
        }
