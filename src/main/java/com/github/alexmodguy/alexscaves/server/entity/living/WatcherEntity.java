...
public class WatcherEntity extends Monster implements IAnimatedEntity, PossessesCamera {
    ...
    private static final EntityDataAccessor<Optional<UUID>> POSSESSED_ENTITY_UUID = SynchedEntityData.defineId(WatcherEntity.class, com.github.alexmodguy.alexscaves.server.entity.ACEntityDataRegistry.OPTIONAL_UUID.get());
    ...
    public boolean canPossessTargetEntity(Entity entity) {
        if (entity instanceof Player player) {
            long timeElapsed = level().getGameTime() - ACRuntimeData.getOrCreate(player).getLong(LAST_POSSESSED_TIME_IDENTIFIER).orElse(0L);
    ...
    public static boolean checkWatcherSpawnRules(EntityType<? extends Monster> entityType, ServerLevelAccessor levelAccessor, EntitySpawnReason mobSpawnType, BlockPos blockPos, RandomSource randomSource) {
