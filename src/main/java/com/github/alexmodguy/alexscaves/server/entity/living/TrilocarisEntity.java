    ...
    public void tick() {
        ...
        if (this.entityData.get(ATTACK_TICK) > 0) {
            ...
            if (biteProgress > 4 && this.getTarget() != null && this.distanceTo(this.getTarget()) < 1.3D) {
                this.getTarget().hurtOrSimulate(damageSources().mobAttack(this), (float) this.getAttribute(Attributes.ATTACK_DAMAGE).getValue());
            }
            if (biteProgress > 0F) {
                biteProgress--;
            }
        ...
        public void tick() {
            ...
            if (target != null && target.isAlive()) {
                ...
                if (TrilocarisEntity.this.distanceTo(target) < 1.2F && cooldown == 0) {
                    TrilocarisEntity.this.doHurtTarget((net.minecraft.server.level.ServerLevel) TrilocarisEntity.this.level(), target);
                    cooldown = 30;
                }
            }
            if (cooldown > 0) {
