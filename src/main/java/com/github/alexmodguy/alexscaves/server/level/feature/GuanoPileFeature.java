...
public class GuanoPileFeature extends Feature<NoneFeatureConfiguration> {
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        ...
        while (pileBottom.getY() > level.getMinY() && level.getBlockState(pileBottom).canBeReplaced()) {
        ...
        for (int x = -radius; x <= radius; x++) {
            for (int z = -radius; z <= radius; z++) {
                ...
                while (!level.getBlockState(side).canBeReplaced() && side.getY() < level.getMaxY()) {
                    side.move(0, 1, 0);
                }
                while (level.getBlockState(side).canBeReplaced() && side.getY() > level.getMinY()) {
                    side.move(0, -1, 0);
                }
