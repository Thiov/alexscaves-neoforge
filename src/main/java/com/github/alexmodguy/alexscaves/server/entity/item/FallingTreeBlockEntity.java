...
public class FallingTreeBlockEntity extends AbstractMovingBlockEntity {
    ...
    public void readAdditionalSaveData(net.minecraft.world.level.storage.ValueInput compound) {
        ...
        this.setFallDirection(Direction.from3DDataValue(com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getByte(compound, "FallDirection")));
        this.setFallProgress(com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getFloat(compound, "FallProgress"));
    }
            ...
            if (!level().isClientSide() && !droppedItems) {
                for (MovingBlockData dataBlock : this.getData()) {
                    ...
                    if (!offset.equals(BlockPos.ZERO)) {
                        ...
                        while (level().getBlockState(fallPos).isAir() && fallPos.getY() > level().getMinY()) {
