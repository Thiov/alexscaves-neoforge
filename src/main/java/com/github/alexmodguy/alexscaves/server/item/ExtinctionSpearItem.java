...
public class ExtinctionSpearItem extends SpearItem {
    ...
    public boolean releaseUsing(ItemStack itemStack, Level level, LivingEntity livingEntity, int useTimeLeft) {
        if (livingEntity instanceof Player player) {
            ...
            if (power > 0.1D) {
                ...
                spearEntity.shootFromRotation(player, player.getXRot(), player.getYRot(), 0.0F, power * 3.5F, 1.0F);
                if (player.getAbilities().instabuild) {
                    spearEntity.pickup = AbstractArrow.Pickup.CREATIVE_ONLY;
                }
                level.addFreshEntity(spearEntity);
                level.playSound(null, spearEntity, ACSoundRegistry.EXTINCTION_SPEAR_THROW.get(), SoundSource.PLAYERS, 1.0F, 1.0F);
                if (!player.getAbilities().instabuild) {
                    itemStack.shrink(1);
                }
    ...
    public void onUseTick(Level level, LivingEntity living, ItemStack stack, int timeUsing) {
        if (timeUsing == getUseDuration(stack, living)) {
            level.playSound(null, living, ACSoundRegistry.EXTINCTION_SPEAR_SUMMON.get(), SoundSource.PLAYERS, 1.0F, 1.0F);
            ...
            for (int i = 0; i < grottoHeads; i++) {
                DinosaurSpiritEntity dinosaurSpirit = ACEntityRegistry.DINOSAUR_SPIRIT.get().create(level, EntitySpawnReason.EVENT);
    ...
    public void hurtEnemy(ItemStack stack, LivingEntity hurtEntity, LivingEntity player) {
        ...
        DinosaurSpiritEntity dinosaurSpirit = ACEntityRegistry.DINOSAUR_SPIRIT.get().create(player.level(), EntitySpawnReason.EVENT);
        if (dinosaurSpirit == null) {
