...
public class GumWormDigRandomlyGoal extends Goal {
    ...
    private Vec3 generateAnyPosition(boolean favoredBlocksOnly) {
        ...
        for (int i = 0; i < 20; i++) {
            ...
            if (check.getY() < entity.level().getMinY() || !entity.level().isLoaded(check)) {
                break;
            }
            if (surface) {
                while (!entity.level().isEmptyBlock(check) && check.getY() < entity.level().getMaxY()) {
                    checkBefore.set(check);
                    check.move(0, 1, 0);
                }
                ...
                while (entity.level().isEmptyBlock(check) && check.getY() > entity.level().getMinY() - 1) {
                    checkBefore.set(check);
                    check.move(0, -1, 0);
                }
                while(check.getY() < entity.level().getMinY() + 1){
                    check.move(0, 1, 0);
                }
