...
public class SackOfSatingItem extends Item {
    ...
    public static int getHunger(ItemStack itemStack) {
        ...
        return com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getInt(compoundtag, "HungerValue");
    }

    public static boolean isChewing(ItemStack itemStack, long gameTimeIn) {
        CompoundTag compoundtag = itemStack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag();
        return compoundtag.contains("ChewTimestamp") && gameTimeIn - com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getLong(compoundtag, "ChewTimestamp") < 30;
    }

    public static boolean isExploding(ItemStack itemStack) {
        CompoundTag compoundtag = itemStack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag();
        return com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getBoolean(compoundtag, "Exploding");
    }

    public static long getFeedTimestamp(ItemStack itemStack) {
        CompoundTag compoundtag = itemStack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag();
        return compoundtag.contains("FeedTimestamp") ? com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getLong(compoundtag, "FeedTimestamp") : -1;
    }
        ...
        if (clickAction != ClickAction.SECONDARY || foodStack.is(ACTagRegistry.RESTRICTED_FROM_SACK_OF_SATING)) {
            ...
            if (!foodStack.isEmpty() && foodStack.has(DataComponents.FOOD)) {
                ...
                if (!craftingRemainder.isEmpty()) {
                    ...
                    containerItem.setCount(foodStack.getCount());
                }
                ...
                if(foodStack.is(Items.HONEY_BOTTLE) || foodStack.getItem() instanceof DrinkableBottledItem){
