...
public class SubterranodonFlightGoal extends Goal {
    ...
    protected Vec3 getPosition() {
        ...
        if (isFlying) {
            if ((entity.timeFlying < 2000 || isLeaderStillGoing() || isOverWaterOrVoid()) && !entity.isOrderedToSit()) {
                ...
                if (com.github.alexmodguy.alexscaves.server.entity.util.MobCompat.hasRestriction(entity) && !entity.horizontalCollision) {
                    return Vec3.atCenterOf(com.github.alexmodguy.alexscaves.server.entity.util.MobCompat.getRestrictCenter(entity));
                }
    ...
    private Vec3 findFlightPos() {
        Vec3 targetVec;
        if (com.github.alexmodguy.alexscaves.server.entity.util.MobCompat.hasRestriction(entity) && com.github.alexmodguy.alexscaves.server.entity.util.MobCompat.getRestrictCenter(entity) != null) {
            float maxRot = 360;
            Vec3 center = Vec3.atCenterOf(com.github.alexmodguy.alexscaves.server.entity.util.MobCompat.getRestrictCenter(entity));
    ...
    private boolean isOverWaterOrVoid() {
        ...
        while (position.getY() > entity.level().getMinY() && entity.level().isEmptyBlock(position) && entity.level().getFluidState(position).isEmpty()) {
            position = position.below();
        }
        return !entity.level().getFluidState(position).isEmpty() || entity.level().getBlockState(position).is(Blocks.VINE) || position.getY() <= entity.level().getMinY();
    }
    ...
    public Vec3 groundPosition(Vec3 airPosition) {
        ...
        boolean flag = false;
        while (ground.getY() < entity.level().getMaxY() && !entity.level().getBlockState(ground).isSolid() && entity.level().getFluidState(ground).isEmpty()){
            ground.move(0, 1, 0);
            flag = true;
        }
        ground.move(0, -1, 0);
        while (ground.getY() > entity.level().getMinY() && !entity.level().getBlockState(ground).isSolid() && entity.level().getFluidState(ground).isEmpty()) {
            ground.move(0, -1, 0);
        }
