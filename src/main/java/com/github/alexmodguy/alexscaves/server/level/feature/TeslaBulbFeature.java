...
public class TeslaBulbFeature extends Feature<NoneFeatureConfiguration> {
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        ...
        if (ceiling) {
            while ((level.getBlockState(generateAt).getFluidState().is(Fluids.WATER) || !level.getBlockState(generateAt).isFaceSturdy(level, generateAt, Direction.DOWN)) && generateAt.getY() < level.getMaxY()) {
                generateAt.move(0, 1, 0);
            }
        } else {
            while ((level.getBlockState(generateAt).getFluidState().is(Fluids.WATER) || !level.getBlockState(generateAt).isFaceSturdy(level, generateAt, Direction.UP)) && generateAt.getY() > level.getMinY()) {
                generateAt.move(0, -1, 0);
            }
        }
        ...
        for (int i = 0; i < 4 + randomsource.nextInt(4); i++) {
            ...
            if (ceiling) {
                while (!level.getBlockState(offset).isFaceSturdy(level, offset, Direction.DOWN) && offset.getY() < level.getMaxY()) {
                    offset = offset.above();
                }
            } else {
                while (!level.getBlockState(offset).isFaceSturdy(level, offset, Direction.UP) && offset.getY() > level.getMinY()) {
                    offset = offset.below();
                }
            }
