...
public class LanternfishEntity extends WaterAnimal implements Bucketable {
    ...
    public static boolean checkLanternfishSpawnRules(EntityType<? extends LivingEntity> type, ServerLevelAccessor level, EntitySpawnReason spawnType, BlockPos pos, RandomSource randomSource) {
    ...
    private class SwimInSchoolGoal extends Goal {
        ...
        public BlockPos findSwimToPos(int range) {
            ...
            while (move.getY() < level().getMaxY() && level().getFluidState(move).is(FluidTags.WATER)) {
                move.move(0, 5, 0);
            }
            surfaceY = move.getY();
            move.set(fish.getX(), fish.getY() - 1, fish.getZ());
            while (move.getY() > level().getMinY() && level().getFluidState(move).is(FluidTags.WATER)) {
                move.move(0, -5, 0);
            }
            floorY = move.getY();
            int oceanHeight = surfaceY - floorY;
            if (com.github.alexmodguy.alexscaves.server.entity.util.LevelCompat.isNight(level())) {
            ...
            for (int i = 0; i < 15; i++) {
                ...
                if (fish.level().getFluidState(blockPos).is(FluidTags.WATER) && blockPos.getY() > level().getMinY() + 1) {
                    return blockPos;
                }
            }
            return base;
    ...
    private class LanternfishMoveControl extends MoveControl {
        ...
        public void tick() {
            if (this.operation == MoveControl.Operation.MOVE_TO && mob.isInWater()) {
