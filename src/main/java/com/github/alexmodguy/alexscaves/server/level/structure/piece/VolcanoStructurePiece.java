...
public class VolcanoStructurePiece extends StructurePiece {
    ...
    public VolcanoStructurePiece(CompoundTag tag) {
        ...
        this.center = new BlockPos(com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getInt(tag, "CPX"), com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getInt(tag, "CPY"), com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getInt(tag, "CPZ"));
        ...
        this.volcanoRadius = com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getInt(tag, "Radius");
        this.volcanoHeight = com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getInt(tag, "Height");
    }
    ...
    private static BoundingBox createBoundingBox(BlockPos chunkCorner) {
        ChunkPos chunkPos = net.minecraft.world.level.ChunkPos.containing(chunkCorner);
        ...
        for (int x = 0; x < 16; x++) {
            for (int z = 0; z < 16; z++) {
                ...
                if(dist < 1.0F){
                    ...
                    int j = 0;
                    while((volcanoReplacesBeneath(checkedGetBlock(level, carve)) || j < 3) && carve.getY() > level.getMinY()){
                        checkedSetBlock(level, carve, ACBlockRegistry.LIMESTONE.get().defaultBlockState());
                        carve.move(0, -1, 0);
                        j++;
                    }
