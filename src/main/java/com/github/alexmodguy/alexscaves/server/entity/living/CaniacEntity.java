...
public class CaniacEntity extends Monster implements IAnimatedEntity, PossessedByLicowitch {
    ...
    public static boolean checkCaniacSpawnRules(EntityType<? extends Monster> entityType, ServerLevelAccessor levelAccessor, EntitySpawnReason mobSpawnType, BlockPos blockPos, RandomSource randomSource) {
    ...
    private void spawnArmSwingParticles(boolean left){
        ...
        if (state.isSolid()) {
            com.github.alexmodguy.alexscaves.server.entity.util.ParticleCompat.addParticle(level(), new BlockParticleOption(ParticleTypes.BLOCK, state), true, armDustPosition.x, ground.getY() + 1, armDustPosition.z, dustMotion.x, dustMotion.y, dustMotion.z);
        }
    }
    ...
    private void hurtMobsFromArmSwing(boolean left, float damageAmount, float knockbackAmount) {
        ...
        for(LivingEntity living : level().getEntitiesOfClass(LivingEntity.class, hurtBox, EntitySelector.NO_CREATIVE_OR_SPECTATOR)){
            if(!living.is(this) && !living.isAlliedTo(this) && living.getType() != this.getType()){
                ...
                if(living.distanceTo(this) < 3.15F && living.hurtOrSimulate(damageSource, damageAmount)){
