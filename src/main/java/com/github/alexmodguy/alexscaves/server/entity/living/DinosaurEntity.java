...
public abstract class DinosaurEntity extends TamableAnimal implements IDancesToJukebox, LaysEggs, IAdvancedPathingMob, RidingMeterMount {
    ...
    public void readAdditionalSaveData(net.minecraft.world.level.storage.ValueInput compound) {
        ...
        this.setHasEgg(com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getBoolean(compound, "Egg"));
        int altSkin = com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getInt(compound, "AltSkin");
        //compatibility with pre 1.1.0 saves
        if (com.github.alexmodguy.alexscaves.server.misc.NbtCompat.contains(compound, "Retro") && com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getBoolean(compound, "Retro")) {
            altSkin = 1;
        }
    ...
    public void onLayEggTick(BlockPos belowEgg, int time) {
        com.github.alexmodguy.alexscaves.server.entity.util.EntityCompat.updateWalkAnimation(this.walkAnimation, 0.5F, 0.4F);
            ...
            for (int i1 = 0; i1 < particleCount; i1++) {
                ...
                if (groundState.isSolid()) {
                    if (level().isClientSide()) {
                        com.github.alexmodguy.alexscaves.server.entity.util.ParticleCompat.addParticle(level(), new BlockParticleOption(ParticleTypes.BLOCK, groundState), true, this.getX() + extraX, ground.getY(), this.getZ() + extraZ, motionX, motionY, motionZ);
                    }
                }
            }
        } else if (b == 78) {
            ...
            if (altSkinFromItem > 0) {
                ...
                if (canOwnerCommand(player)) {
                    ...
                    if (this.getType() == ACEntityRegistry.SUBTERRANODON.get() && this.canAddPassenger(player)) {
                        com.github.alexmodguy.alexscaves.server.entity.util.EntityCompat.moveTo(this, this.getX(), this.getY() + player.getBbHeight() + 0.5F, this.getZ());
                    }
    ...
    public boolean startRiding(Entity entity, boolean force) {
        boolean flag = super.startRiding(entity, force, true);
