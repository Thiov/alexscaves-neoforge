...
public class VesperEntity extends Monster implements IAnimatedEntity, UnderzealotSacrifice {
    ...
    public void tick() {
        ...
        if (isBeingSacrificed && this.isPassenger() && !level().isClientSide()) {
            ...
            if (sacrificeTime < 0) {
                ...
                this.stopRiding();
                ForsakenEntity forsakenEntity = com.github.alexmodguy.alexscaves.server.entity.util.MobCompat.convertTo(this, ACEntityRegistry.FORSAKEN.get(), true);
    ...
    public static boolean checkVesperSpawnRules(EntityType<? extends Monster> entityType, ServerLevelAccessor levelAccessor, EntitySpawnReason mobSpawnType, BlockPos blockPos, RandomSource randomSource) {
        if (checkMonsterSpawnRules(entityType, levelAccessor, mobSpawnType, blockPos, randomSource)) {
            BlockPos.MutableBlockPos above = new BlockPos.MutableBlockPos();
            above.set(blockPos);
            int k = 0;
            while (levelAccessor.isEmptyBlock(above) && above.getY() < levelAccessor.getMaxY()) {
                above.move(0, 1, 0);
                k++;
                if (k > 5) {
                    return true;
