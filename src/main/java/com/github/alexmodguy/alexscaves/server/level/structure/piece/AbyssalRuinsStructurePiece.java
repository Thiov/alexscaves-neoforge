...
public class AbyssalRuinsStructurePiece extends TemplateStructurePiece {
    ...
    public AbyssalRuinsStructurePiece(StructureTemplateManager manager, Identifier resourceLocation, BlockPos pos, Rotation rotation) {
    ...
    private int getHeight(BlockPos blockPos, BlockGetter level, BlockPos pos) {
        ...
        for (BlockPos blockpos : BlockPos.betweenClosed(blockPos, pos)) {
            ...
            for (FluidState fluidstate = level.getFluidState(blockpos$mutableblockpos); (blockstate.isAir() || fluidstate.is(FluidTags.WATER) || blockstate.is(BlockTags.ICE)) && k1 > level.getMinY() + 1; fluidstate = level.getFluidState(blockpos$mutableblockpos)) {
                --k1;
    ...
    private void spawnSubmarine(ServerLevelAccessor level, BlockPos pos, boolean totaled) {
        SubmarineEntity submarine = ACEntityRegistry.SUBMARINE.get().create(level.getLevel(), net.minecraft.world.entity.EntitySpawnReason.EVENT);
        while (level.getBlockState(pos).getFluidState().isEmpty() && !level.isEmptyBlock(pos) && pos.getY() < level.getMaxY()) {
            pos = pos.above();
        }
