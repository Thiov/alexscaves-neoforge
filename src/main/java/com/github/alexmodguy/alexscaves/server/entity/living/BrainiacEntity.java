...
public class BrainiacEntity extends Monster implements IAnimatedEntity {
    ...
    public void tick() {
        ...
        if (!level().isClientSide() && this.hasBarrel()) {
            if (this.getAnimation() == ANIMATION_DRINK_BARREL && this.getAnimationTick() >= 60) {
                ...
                this.heal(10);
                this.addEffect(new MobEffectInstance(MobEffects.STRENGTH, 400, 0));
                this.addEffect(new MobEffectInstance(MobEffects.RESISTANCE, 400, 0));
            ...
            if (this.getAnimation() == ANIMATION_THROW_BARREL && this.getAnimationTick() >= 15) {
                ...
                if (attackTarget != null && attackTarget.isAlive()) {
                    ...
                    ThrownWasteDrumEntity wasteDrumEntity = ACEntityRegistry.THROWN_WASTE_DRUM.get().create(level(), net.minecraft.world.entity.EntitySpawnReason.EVENT);
        ...
        if (tongueTarget instanceof LivingEntity living) {
            if (this.shootTongueAmount >= 5F) {
                ...
                tongueTarget.hurtOrSimulate(damageSources().mobAttack(this), 4);
    ...
    public void readAdditionalSaveData(net.minecraft.world.level.storage.ValueInput compound) {
        ...
        this.setHasBarrel(com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getBoolean(compound, "HasBarrel"));
    }
    ...
    protected void dropEquipment() {
        ...
        if (this.hasBarrel() && shouldDropBlocks()) {
            com.github.alexmodguy.alexscaves.server.entity.util.EntityCompat.spawnAtLocation(this, new ItemStack(ACBlockRegistry.WASTE_DRUM.get()));
        }
    }
    ...
    private class MeleeGoal extends Goal {
        ...
        private void checkAndDealDamage(LivingEntity target, float damageMultiplier) {
            if (BrainiacEntity.this.hasLineOfSight(target) && BrainiacEntity.this.distanceTo(target) < BrainiacEntity.this.getBbWidth() + target.getBbWidth() + 2.0D) {
                target.hurtOrSimulate(damageSources().mobAttack(BrainiacEntity.this), (float) BrainiacEntity.this.getAttribute(Attributes.ATTACK_DAMAGE).getValue() * damageMultiplier);
