...
public class TubeWormFeature extends Feature<NoneFeatureConfiguration> {
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        ...
        while (!level.getBlockState(ventBottom).getFluidState().isEmpty() && ventBottom.getY() > level.getMinY()) {
        ...
        if (ventBottom.getY() < level.getSeaLevel() - 30 && level.getBlockState(ventBottom.below()).equals(Blocks.TUFF.defaultBlockState())) {
            for (int i = 0; i < 4 + randomsource.nextInt(4); i++) {
                ...
                if (randomState.isFaceSturdy(level, randomPos, randomDirection.getOpposite())) {
                    ...
                    while (level.getBlockState(wormAt).isFaceSturdy(level, wormAt.above(), Direction.DOWN) && wormAt.getY() < level.getMaxY()) {
                        wormAt = wormAt.above();
                    }
                    while (!level.getBlockState(wormAt).getFluidState().isEmpty() && wormAt.getY() > level.getMinY()) {
                        wormAt = wormAt.below();
                    }
