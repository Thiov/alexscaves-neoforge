...
public class TremorzillaFollowOwnerGoal extends Goal {
    ...
    private boolean maybeTeleportTo(int x, int y, int z) {
        if (Math.abs((double)x - this.owner.getX()) < 6.0D && Math.abs((double)z - this.owner.getZ()) < 6.0D) {
            ...
            com.github.alexmodguy.alexscaves.server.entity.util.EntityCompat.moveTo(tremorzilla, (double)x + 0.5D, (double)y, (double)z + 0.5D, this.tremorzilla.getYRot(), this.tremorzilla.getXRot());
            this.tremorzilla.getNavigation().stop();
            return true;
        }
    }
