...
public class DeepOneWanderGoal extends Goal {
    ...
    public boolean canUse() {
        ...
        return mob.isInWater() && (target == null || !target.isAlive()) && (chance == 0 || mob.getRandom().nextInt(chance) == 0) && !mob.isTradingLocked();
    }
    ...
    public BlockPos findSwimToPos() {
        ...
        while (move.getY() < mob.level().getMaxY() && mob.level().getFluidState(move).is(FluidTags.WATER)) {
            move.move(0, 5, 0);
        }
        surfaceY = move.getY();
        ...
        for (int i = 0; i < 15; i++) {
            ...
            if (mob.level().getFluidState(blockPos).is(FluidTags.WATER) && !isTargetBlocked(Vec3.atCenterOf(blockPos)) && blockPos.getY() > mob.level().getMinY() + 1) {
                if (groundTarget) {
                    while (groundTarget && mob.level().getFluidState(blockPos.below()).is(FluidTags.WATER) && blockPos.getY() > mob.level().getMinY()) {
