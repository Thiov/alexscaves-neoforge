...
public class LicowitchEntity extends Monster implements IAnimatedEntity {
    ...
    private static final EntityDataAccessor<Optional<UUID>> POSSESSED_UUID_0 = SynchedEntityData.defineId(LicowitchEntity.class, com.github.alexmodguy.alexscaves.server.entity.ACEntityDataRegistry.OPTIONAL_UUID.get());
    private static final EntityDataAccessor<Optional<UUID>> POSSESSED_UUID_1 = SynchedEntityData.defineId(LicowitchEntity.class, com.github.alexmodguy.alexscaves.server.entity.ACEntityDataRegistry.OPTIONAL_UUID.get());
    private static final EntityDataAccessor<Optional<UUID>> POSSESSED_UUID_2 = SynchedEntityData.defineId(LicowitchEntity.class, com.github.alexmodguy.alexscaves.server.entity.ACEntityDataRegistry.OPTIONAL_UUID.get());
    ...
    // Lazy: 26.1 binds item components after registration, so an ItemStack built in this class's
    // static initializer would throw "Components not bound yet" during early bootstrap.
    ...
    public static boolean checkLicowitchSpawnRules(EntityType<? extends Monster> entityType, ServerLevelAccessor levelAccessor, EntitySpawnReason mobSpawnType, BlockPos blockPos, RandomSource randomSource) {
    ...
    public static boolean isWithinTowerSpawnBounds(ServerLevelAccessor level, BlockPos pos) {
        ResourceKey<Structure> structureKey = ResourceKey.create(Registries.STRUCTURE, Identifier.fromNamespaceAndPath(AlexsCaves.MODID, "licowitch_tower"));
        Structure structure = level.registryAccess().lookupOrThrow(Registries.STRUCTURE).getValue(structureKey);
        if (structure != null) {
    ...
    public void tick() {
        ...
        if (this.getAnimation() == ANIMATION_EAT && this.getAnimationTick() < 90 && this.getAnimationTick() % 4 == 0) {
            this.spawnItemParticles(this.getItemInHand(InteractionHand.MAIN_HAND), 2);
        }
        ...
        if (!level().isClientSide()) {
            ...
            if (level() instanceof ServerLevel) {
                for (UUID possessedId : possessedIds) {
                    Entity possessedEntity = ((ServerLevel) level()).getEntity(possessedId);
                    if (possessedEntity != null) {
                        possessedEntity.hurtOrSimulate(this.damageSources().magic(), 300);
                    }
                }
            }
        }
    ...
    public void addAdditionalSaveData(net.minecraft.world.level.storage.ValueOutput compoundTag) {
        ...
        com.github.alexmodguy.alexscaves.server.misc.NbtCompat.putUUIDList(compoundTag, "Possessed", list);
    ...
    public void readAdditionalSaveData(net.minecraft.world.level.storage.ValueInput compoundTag) {
        ...
        java.util.List<java.util.UUID> listtag = com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getUUIDList(compoundTag, "Possessed");
        for (int i = 0; i < listtag.size(); ++i) {
            this.addPossessedUUID(listtag.get(i));
        }
        if (com.github.alexmodguy.alexscaves.server.misc.NbtCompat.contains(compoundTag, "CrucibleX") && com.github.alexmodguy.alexscaves.server.misc.NbtCompat.contains(compoundTag, "CrucibleY") && com.github.alexmodguy.alexscaves.server.misc.NbtCompat.contains(compoundTag, "CrucibleZ")) {
            this.setLastCruciblePos(new BlockPos(com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getInt(compoundTag, "CrucibleX"), com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getInt(compoundTag, "CrucibleY"), com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getInt(compoundTag, "CrucibleZ")));
        }
        teleportCooldown = com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getInt(compoundTag, "TeleportCooldown");
    }

    
    protected float getEquipmentDropChanceCompat(EquipmentSlot slot) {
        return slot.isArmor() ? super.getDropChances().byEquipment(slot) : 0.0F;
    }
    ...
    public Mob createRandomPossessedMob() {
        ...
        if (chance < 0.1F) {
            return ACEntityRegistry.CANDICORN.get().create(level(), net.minecraft.world.entity.EntitySpawnReason.EVENT);
        } else if (chance < 0.2F) {
            return ACEntityRegistry.GUMMY_BEAR.get().create(level(), net.minecraft.world.entity.EntitySpawnReason.EVENT);
        } else if (chance < 0.5F) {
            return ACEntityRegistry.GUMBEEPER.get().create(level(), net.minecraft.world.entity.EntitySpawnReason.EVENT);
        } else if (chance < 0.65F) {
            return ACEntityRegistry.CARAMEL_CUBE.get().create(level(), net.minecraft.world.entity.EntitySpawnReason.EVENT);
        } else if (chance < 0.75F) {
            return ACEntityRegistry.GINGERBREAD_MAN.get().create(level(), net.minecraft.world.entity.EntitySpawnReason.EVENT);
        } else {
            return ACEntityRegistry.CANIAC.get().create(level(), net.minecraft.world.entity.EntitySpawnReason.EVENT);
        }
    }
