...
public class WaterBoltEntity extends Projectile {
    ...
    private static final EntityDataAccessor<Optional<UUID>> ARC_TOWARDS_ENTITY_UUID = SynchedEntityData.defineId(WaterBoltEntity.class, com.github.alexmodguy.alexscaves.server.entity.ACEntityDataRegistry.OPTIONAL_UUID.get());
    ...
    public void tick() {
        ...
        if (!level().isClientSide()) {
            ...
            for (int j = 0; j < 3 + random.nextInt(2); ++j) {
                this.level().addParticle(this.isInWater() || isBubbling() ? ParticleTypes.BUBBLE_COLUMN_UP : ParticleTypes.FALLING_WATER, this.getRandomX(0.5D), this.getRandomY(), this.getRandomZ(0.5D), 0, -0.1F, 0);
            }
        }
        if(wooshSoundTime <= 0){
    ...
    public void remove(Entity.RemovalReason removalReason) {
        ...
        if (!spawnedSplash && level() instanceof ServerLevel serverLevel) {
            ...
            while (level().isEmptyBlock(pos) && pos.getY() > level().getMinY()) {
                pos = pos.below();
            }
    ...
    private void damageMobs() {
        ...
        for (LivingEntity entity : this.level().getEntitiesOfClass(LivingEntity.class, bashBox)) {
            if (!isAlliedTo(entity) && !(entity instanceof DeepOneBaseEntity) && (owner == null || !entity.is(owner) && !entity.isAlliedTo(owner))) {
                lastHitMob = entity;
                entity.hurtOrSimulate(source, 3.0F);
    ...
    protected void readAdditionalSaveData(net.minecraft.world.level.storage.ValueInput tag) {
        this.setBubbling(com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getBoolean(tag, "Bubbling"));
