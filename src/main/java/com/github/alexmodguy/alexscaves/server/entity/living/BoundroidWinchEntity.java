...
public class BoundroidWinchEntity extends Monster {
    ...
    private static final EntityDataAccessor<Optional<UUID>> HEAD_UUID = SynchedEntityData.defineId(BoundroidWinchEntity.class, com.github.alexmodguy.alexscaves.server.entity.ACEntityDataRegistry.OPTIONAL_UUID.get());
    ...
    public boolean hurtServer(net.minecraft.server.level.ServerLevel serverLevel, DamageSource source, float damage) {
        ...
        if (com.github.alexmodguy.alexscaves.server.entity.util.EntityCompat.isInvulnerableTo(this, source)) {
            return false;
        }
        if (body != null && !com.github.alexmodguy.alexscaves.server.entity.util.EntityCompat.isInvulnerableTo(body, source)) {
            boolean flag = body.hurtOrSimulate(source, damage);
            if (flag) {
    ...
    public void addAdditionalSaveData(net.minecraft.world.level.storage.ValueOutput compound) {
        ...
        if (this.getHeadUUID() != null) {
            com.github.alexmodguy.alexscaves.server.misc.NbtCompat.putUUID(compound, "BodyUUID", this.getHeadUUID());
        }
    }
    ...
    private boolean hasBlockAbove() {
        if(!this.level().canSeeSky(this.blockPosition())){
            ...
            while (mutableBlockPos.getY() < this.level().getMaxY()){
    ...
    public BlockPos getCeilingOf(BlockPos usPos) {
        while (!level().getBlockState(usPos).isFaceSturdy(level(), usPos, Direction.DOWN) && usPos.getY() < level().getMaxY()) {
            usPos = usPos.above();
        }
        return usPos;
    }
