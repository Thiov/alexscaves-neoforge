...
public class CorrodentAttackGoal extends Goal {
    ...
    private Vec3 generateEvadePosition(BlockPos around) {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            if (!entity.level().isLoaded(check) || check.getY() < entity.level().getMinY()) {
                break;
            }
            while (entity.level().isEmptyBlock(check) && entity.level().isLoaded(check) && check.getY() > entity.level().getMinY() - 1) {
                check.move(0, -1, 0);
            }
    ...
    private void checkAndDealDamage(LivingEntity target, float multiplier) {
        if (entity.hasLineOfSight(target) && entity.distanceTo(target) < entity.getBbWidth() + target.getBbWidth() + 1.0D) {
            ...
            target.hurtOrSimulate(target.damageSources().mobAttack(entity), f);
            ...
            if (entity != null) {
                ...
                entity.hurtOrSimulate(target.damageSources().mobAttack(this.entity), f * 0.5F);
            }
            burrowing = true;
        }
    }
