...
public class GloomothEntity extends PathfinderMob implements UnderzealotSacrifice {
    ...
    public void tick() {
        ...
        if (isBeingSacrificed && !level().isClientSide()) {
            ...
            if (sacrificeTime < 0) {
                ...
                this.stopRiding();
                WatcherEntity watcherEntity = com.github.alexmodguy.alexscaves.server.entity.util.MobCompat.convertTo(this, ACEntityRegistry.WATCHER.get(), true);
    ...
    protected void dropFromLootTable(DamageSource damageSource, boolean idk) {
        if(!(damageSource.getEntity() instanceof ForsakenEntity || damageSource.getEntity() instanceof VesperEntity)){
            super.dropFromLootTable((net.minecraft.server.level.ServerLevel) level(), damageSource, idk);
        }
    }
    ...
    public static boolean canMonsterSpawnInLight(EntityType<GloomothEntity> entityType, ServerLevelAccessor levelAccessor, EntitySpawnReason mobSpawnType, BlockPos blockPos, RandomSource randomSource) {
    ...
    public static <T extends Mob> boolean checkGloomothSpawnRules(EntityType<GloomothEntity> entityType, ServerLevelAccessor iServerWorld, EntitySpawnReason reason, BlockPos pos, RandomSource random) {
        if (canMonsterSpawnInLight(entityType, iServerWorld, reason, pos, random)) {
            BlockPos.MutableBlockPos above = new BlockPos.MutableBlockPos();
            above.set(pos);
            int k = 0;
            while (iServerWorld.isEmptyBlock(above) && above.getY() < iServerWorld.getMaxY()) {
                above.move(0, 1, 0);
                k++;
                if (k > 4) {
                    return true;
    ...
    private void doInitialPosing(LevelAccessor world) {
        ...
        int k = 0;
        while (world.isEmptyBlock(above) && above.getY() < level().getMaxY() && k < upBy) {
            above = above.above();
            k++;
        }
        this.setFlying(true);
