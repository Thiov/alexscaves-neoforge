...
public class SubterranodonFollowOwnerGoal extends Goal {
    ...
    private boolean tryToTeleportToLocation(int x, int y, int z) {
        if (Math.abs((double) x - this.owner.getX()) < 2.0D && Math.abs((double) z - this.owner.getZ()) < 2.0D) {
            ...
            com.github.alexmodguy.alexscaves.server.entity.util.EntityCompat.moveTo(subterranodon, (double) x + 0.5D, (double) y, (double) z + 0.5D, this.subterranodon.getYRot(), this.subterranodon.getXRot());
            this.navigator.stop();
            return true;
        }
    }
