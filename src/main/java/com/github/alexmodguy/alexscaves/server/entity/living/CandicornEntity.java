...
public class CandicornEntity extends TamableAnimal implements KeybindUsingMount, IAnimatedEntity, PlayerRideableJumping, Saddleable, RidingMeterMount, PossessedByLicowitch, RendersOwnRider {
    ...
    public boolean causeFallDamage(float f1, float f2, DamageSource damageSource) {
        net.neoforged.neoforge.event.entity.living.LivingFallEvent ret = net.neoforged.neoforge.common.CommonHooks.onLivingFall(this, f1, f2);
        if (ret == null || ret.isCanceled()) return false;
        f1 = (float) ret.getDistance();
        f2 = ret.getDamageMultiplier();
