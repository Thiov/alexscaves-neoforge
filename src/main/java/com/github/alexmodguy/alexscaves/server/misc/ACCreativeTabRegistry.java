...
public class ACCreativeTabRegistry {
    ...
    private static void add(CreativeModeTab.Output tab, ItemLike itemLike) {
        if (itemLike instanceof CustomTabBehavior customTabBehavior) {
            ...
            // 26.1: CreativeModeTab.Output requires a stack of exactly 1 and rejects empty stacks.
            // A block registered without a BlockItem resolves to an empty stack (asItem() == AIR);
            // older versions silently skipped these, so guard to preserve that behavior.
            net.minecraft.world.item.ItemStack stack = new net.minecraft.world.item.ItemStack(itemLike);
            if (!stack.isEmpty()) {
                tab.accept(stack);
            }
        }
