...
public class NuclearExplosionEntity extends Entity {
    ...
    public static final TicketController TICKET_CONTROLLER = new TicketController(Identifier.fromNamespaceAndPath(AlexsCaves.MODID, "nuclear_explosion"));
        ...
        if (tickCount > 40 && destroyingChunks.isEmpty()) {
            ...
            for (LivingEntity entity : this.level().getEntitiesOfClass(LivingEntity.class, killBox)) {
                ...
                if (damage > 0) {
                    ...
                    if(damage > 0){
                        entity.hurtOrSimulate(isIntentionalGameDesign() ? ACDamageTypes.causeIntentionalGameDesign(level().registryAccess()) : ACDamageTypes.causeNukeDamage(level().registryAccess()), damage);
                    }
                }
    ...
    private void removeChunk(int radius) {
        ...
        if (dummyExplosion == null) {
            dummyExplosion = com.github.alexmodguy.alexscaves.server.misc.ExplosionCompat.createDummy(level(), this, this.getX(), this.getY(), this.getZ(), 10.0F);
        }
        for (int x = 0; x < 16; x++) {
            for (int z = 0; z < 16; z++) {
                for (int y = 15; y >= 0; y--) {
                    boolean canSetToFire = false;
                    carve.set(chunkCorner.getX() + x, Mth.clamp(chunkCorner.getY() + y, level().getMinY(), level().getMaxY()), chunkCorner.getZ() + z);
    ...
    protected void readAdditionalSaveData(net.minecraft.world.level.storage.ValueInput compoundTag) {
        loadingChunks = com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getBoolean(compoundTag, "WasLoadingChunks");
    }
