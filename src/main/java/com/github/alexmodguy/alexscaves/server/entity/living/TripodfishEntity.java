...
public class TripodfishEntity extends WaterAnimal implements Bucketable {
    ...
    public static boolean checkTripodfishSpawnRules(EntityType<? extends LivingEntity> type, ServerLevelAccessor level, EntitySpawnReason spawnType, BlockPos pos, RandomSource randomSource) {
    ...
    private class WanderGoal extends Goal {
        ...
        public void stop() {
            ...
            int down = 0;
            while (TripodfishEntity.this.level().getFluidState(ground).is(FluidTags.WATER) && down < 3 && ground.getY() > level().getMinY()) {
                ground = ground.below();
                down++;
            }
        ...
        public BlockPos findWaterBlock() {
            ...
            for (int i = 0; i < 15; i++) {
                ...
                if (TripodfishEntity.this.level().getFluidState(blockPos).is(FluidTags.WATER) && blockPos.getY() > level().getMinY()) {
                    result = blockPos;
                }
            }
            return result;
        ...
        protected Vec3 getPosition() {
            ...
            if (TripodfishEntity.this.isInWater()) {
                if (water == null) {
                    return null;
                }
                while (TripodfishEntity.this.level().getFluidState(water.below()).is(FluidTags.WATER) && water.getY() > level().getMinY() + 1) {
                    water = water.below();
                }
