...
public class GingerbreadHousePiece extends UndergroundTemplateStructurePiece {
    ...
    public static final Identifier TOWN_CENTER_TEMPLATE = Identifier.fromNamespaceAndPath(AlexsCaves.MODID, "gingerbread_town_center");
    public static final Identifier[] HOUSE_TEMPLATES = new Identifier[]{
            Identifier.fromNamespaceAndPath(AlexsCaves.MODID, "gingerbread_house_0"),
            Identifier.fromNamespaceAndPath(AlexsCaves.MODID, "gingerbread_house_1"),
            Identifier.fromNamespaceAndPath(AlexsCaves.MODID, "gingerbread_house_2"),
            Identifier.fromNamespaceAndPath(AlexsCaves.MODID, "gingerbread_house_3"),
            Identifier.fromNamespaceAndPath(AlexsCaves.MODID, "gingerbread_house_4"),
            Identifier.fromNamespaceAndPath(AlexsCaves.MODID, "gingerbread_house_5"),
            Identifier.fromNamespaceAndPath(AlexsCaves.MODID, "gingerbread_house_6"),
            Identifier.fromNamespaceAndPath(AlexsCaves.MODID, "gingerbread_house_7"),
            Identifier.fromNamespaceAndPath(AlexsCaves.MODID, "gingerbread_house_8"),
            Identifier.fromNamespaceAndPath(AlexsCaves.MODID, "gingerbread_house_9")
    };

    public GingerbreadHousePiece(StructureTemplateManager manager, Identifier resourceLocation, BlockPos pos, Rotation rotation) {
    ...
    public GingerbreadHousePiece(StructureTemplateManager manager, CompoundTag tag) {
        super(ACStructurePieceRegistry.GINGERBREAD_HOUSE.get(), tag, manager, (x) -> {
            return makeSettings(Rotation.valueOf(com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getString(tag, "Rotation")));
        });
    }
    ...
    public void postProcess(WorldGenLevel level, StructureManager structureManager, ChunkGenerator chunkGenerator, RandomSource randomSource, BoundingBox boundingBox, ChunkPos chunkPos, BlockPos structurePos) {
        ...
        for (BlockPos pos : supportsNeededBelow) {
            ...
            if(boundingBox.isInside(groundedInBox)){
                ...
                while ((!level.getBlockState(grounded).getFluidState().isEmpty() || !level.getBlockState(grounded).isCollisionShapeFullBlock(level, grounded)) && grounded.getY() > level.getMinY() && yDown < maxYDown) {
