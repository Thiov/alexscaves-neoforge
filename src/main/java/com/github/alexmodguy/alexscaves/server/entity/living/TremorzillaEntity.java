...
public class TremorzillaEntity extends DinosaurEntity implements KeybindUsingMount, IAnimatedEntity, ShakesScreen, KaijuMob, ActivatesSirens, ITallWalker, RendersOwnRider {
    ...
    public boolean breakBlocksAround(Vec3 center, float radius, boolean square, boolean triggerExplosions, float dropChance) {
        if (this.isBaby() || !(this.level() instanceof net.minecraft.server.level.ServerLevel acServerLevel && net.neoforged.neoforge.event.EventHooks.canEntityGrief(acServerLevel, this)) || level().isClientSide()) {
            return false;
        }
        boolean flag = false;
