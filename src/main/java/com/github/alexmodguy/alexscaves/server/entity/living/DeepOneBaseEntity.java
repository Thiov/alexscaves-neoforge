...
public abstract class DeepOneBaseEntity extends PathfinderMob implements IAnimatedEntity {
    ...
    public static boolean checkDeepOneSpawnRules(EntityType entityType, ServerLevelAccessor level, EntitySpawnReason mobSpawnType, BlockPos blockPos, RandomSource randomSource) {
        if (!level.getFluidState(blockPos.below()).is(FluidTags.WATER)) {
            return false;
        } else {
            boolean flag = level.getDifficulty() != Difficulty.PEACEFUL && Monster.isDarkEnoughToSpawn(level, blockPos, randomSource) && (mobSpawnType == EntitySpawnReason.SPAWNER || level.getFluidState(blockPos).is(FluidTags.WATER));
            ...
            if (altarPos != null) {
                ...
                if (this.getAnimationTick() > getTradingAnimation().getDuration() - 10) {
                    if (level().getBlockEntity(altarPos) instanceof AbyssalAltarBlockEntity altar && !spawnedLootItem) {
                        ...
                        for (Player player : com.github.alexmodguy.alexscaves.server.entity.util.LevelCompat.getNearbyPlayers(level(), TargetingConditions.forNonCombat().range(advancementRange), this, this.getBoundingBox().inflate(advancementRange))) {
    ...
    public void addAdditionalSaveData(net.minecraft.world.level.storage.ValueOutput compound) {
        ...
        if (!swappedItem.isEmpty()) {
            com.github.alexmodguy.alexscaves.server.misc.NbtCompat.putItemStack(compound, "SwappedItem", swappedItem);
        }
        compound.putBoolean("ConchSummoned", this.isSummoned());
        if (summonerUUID != null) {
            com.github.alexmodguy.alexscaves.server.misc.NbtCompat.putUUID(compound, "ConchUUID", summonerUUID);
    ...
    public void readAdditionalSaveData(net.minecraft.world.level.storage.ValueInput compound) {
        ...
        if (com.github.alexmodguy.alexscaves.server.misc.NbtCompat.contains(compound, "AltarX") && com.github.alexmodguy.alexscaves.server.misc.NbtCompat.contains(compound, "AltarY") && com.github.alexmodguy.alexscaves.server.misc.NbtCompat.contains(compound, "AltarZ")) {
            this.setLastAltarPos(new BlockPos(com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getInt(compound, "AltarX"), com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getInt(compound, "AltarY"), com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getInt(compound, "AltarZ")));
        }
        if (com.github.alexmodguy.alexscaves.server.misc.NbtCompat.contains(compound, "SwappedWeapon")) {
            swappedItem = com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getItemStack(compound, "SwappedWeapon");
        }
        this.setSummoned(com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getBoolean(compound, "ConchSummoned"));
        if (com.github.alexmodguy.alexscaves.server.misc.NbtCompat.contains(compound, "ConchUUID")) {
            this.summonerUUID = com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getUUID(compound, "ConchUUID");
            this.setSummonTime(com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getInt(compound, "ConchTime"));
        }
    }
    ...
    public void addReputation(UUID playerUUID, int amount) {
        ...
        if (DeepOneReaction.fromReputation(prev) != newReaction) {
            ...
            if (player != null) {
                ...
                player.sendOverlayMessage(Component.translatable("entity.alexscaves.deep_one.reaction_" + newReaction.toString().toLowerCase(Locale.ROOT)));
            }
        }
    }
    ...
    protected void checkAndDealMeleeDamage(LivingEntity target, float multiplier, float knockback) {
        if (this.hasLineOfSight(target) && this.distanceTo(target) < this.getBbWidth() + target.getBbWidth() + 5.0D) {
            ...
            target.hurtOrSimulate(damageSources().mobAttack(this), f);
            ...
            if (entity != null) {
                ...
                entity.hurtOrSimulate(damageSources().mobAttack(this), f);
            }
        }
    }
    ...
    public boolean isAlliedToCompat(Entity entityIn) {
