...
public class UnderzealotEntity extends Monster implements PackAnimal, IAnimatedEntity {
    ...
    public void tick() {
        ...
        if (!level().isClientSide()) {
            ...
            if (this.isBuried() && buriedProgress >= 20.0F) {
                if (reemergeTime-- < 0) {
                    com.github.alexmodguy.alexscaves.server.entity.util.EntityCompat.moveTo(this, remergePos.getX() + 0.5F, remergePos.getY(), remergePos.getZ() + 0.5F);
            ...
            if (stateOn.isSolid()) {
                for (int i = 0; i < 3; i++) {
                    com.github.alexmodguy.alexscaves.server.entity.util.ParticleCompat.addParticle(level(), new BlockParticleOption(ParticleTypes.BLOCK, stateOn), true, this.getRandomX(0.8F), this.getY(), this.getRandomZ(0.8F), random.nextFloat() - 0.5F, random.nextFloat() + 0.5F, random.nextFloat() - 0.5F);
                }
            }
            if (isBuried()) {
    ...
    public BlockPos findReemergePos(BlockPos origin, int range) {
        for (int i = 0; i < 15; i++) {
            ...
            while (!level().isEmptyBlock(blockPos) && blockPos.getY() < level().getMaxY()) {
                blockPos = blockPos.above();
            }
            while (level().isEmptyBlock(blockPos.below()) && blockPos.getY() > level().getMinY()) {
    ...
    public static boolean checkUnderzealotSpawnRules(EntityType<? extends Monster> entityType, ServerLevelAccessor levelAccessor, EntitySpawnReason mobSpawnType, BlockPos blockPos, RandomSource randomSource) {
    ...
    private void spawnReinforcements(ServerLevelAccessor worldIn) {
        for (int i = 0; i < 3 + random.nextInt(2); i++) {
            UnderzealotEntity friend = ACEntityRegistry.UNDERZEALOT.get().create(worldIn.getLevel(), net.minecraft.world.entity.EntitySpawnReason.EVENT);
    ...
    public void readAdditionalSaveData(net.minecraft.world.level.storage.ValueInput compound) {
        ...
        this.sacrificeCooldown = com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getInt(compound, "SacrificeCooldown");
        this.setBuried(com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getBoolean(compound, "Buried"));
        if (com.github.alexmodguy.alexscaves.server.misc.NbtCompat.contains(compound, "RX") && com.github.alexmodguy.alexscaves.server.misc.NbtCompat.contains(compound, "RY") && com.github.alexmodguy.alexscaves.server.misc.NbtCompat.contains(compound, "RZ")) {
            this.remergePos = new BlockPos(com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getInt(compound, "RX"), com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getInt(compound, "RY"), com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getInt(compound, "RZ"));
        }
        if (com.github.alexmodguy.alexscaves.server.misc.NbtCompat.contains(compound, "SX") && com.github.alexmodguy.alexscaves.server.misc.NbtCompat.contains(compound, "SY") && com.github.alexmodguy.alexscaves.server.misc.NbtCompat.contains(compound, "SZ")) {
            this.setLastSacrificePos(new BlockPos(com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getInt(compound, "SX"), com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getInt(compound, "SY"), com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getInt(compound, "SZ")));
        }
    }
