...
public class SeaPigEntity extends WaterAnimal implements Bucketable {
    ...
    public static final ResourceKey<LootTable> DIGESTION_LOOT_TABLE = ResourceKey.create(Registries.LOOT_TABLE, 
        Identifier.fromNamespaceAndPath("alexscaves", "gameplay/sea_pig_digestion"));
    ...
    public static boolean checkSeaPigSpawnRules(EntityType<? extends LivingEntity> type, ServerLevelAccessor level, EntitySpawnReason spawnType, BlockPos pos, RandomSource randomSource) {
    ...
    private void digestItem() {
        if (!level().isClientSide()) {
            ...
            items.forEach(itemStack -> com.github.alexmodguy.alexscaves.server.entity.util.EntityCompat.spawnAtLocation(this, itemStack));
        }
    ...
    private class WanderGoal extends Goal {
        ...
        public BlockPos findWaterBlock() {
            ...
            for (int i = 0; i < 15; i++) {
                ...
                if (SeaPigEntity.this.level().getFluidState(blockPos).is(FluidTags.WATER) && blockPos.getY() > level().getMinY() + 1) {
                    result = blockPos;
                }
            }
            return result;
        ...
        protected Vec3 getPosition() {
            ...
            if (SeaPigEntity.this.isInWater()) {
                ...
                while (SeaPigEntity.this.level().getFluidState(water.below()).is(FluidTags.WATER) && water.getY() > level().getMinY() + 1) {
                    water = water.below();
                }
                water = water.above();
