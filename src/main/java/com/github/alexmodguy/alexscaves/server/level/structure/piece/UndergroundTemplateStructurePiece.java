...
public abstract class UndergroundTemplateStructurePiece extends TemplateStructurePiece {
    ...
    public UndergroundTemplateStructurePiece(StructurePieceType structurePieceType, int i, StructureTemplateManager structureTemplateManager, Identifier resourceLocation, String id, StructurePlaceSettings settings, BlockPos blockPos) {
        super(structurePieceType, i, structureTemplateManager, resourceLocation, id, settings, blockPos);
    }

    public UndergroundTemplateStructurePiece(StructurePieceType structurePieceType, CompoundTag compoundTag, StructureTemplateManager structureTemplateManager, Function<Identifier, StructurePlaceSettings> function) {
        super(structurePieceType, compoundTag, structureTemplateManager, function);
        this.placedInCaveFlag = com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getBoolean(compoundTag, "PlacedInCave");
    }
    ...
    public void postProcess(WorldGenLevel worldGenLevel, StructureManager structureManager, ChunkGenerator chunkGenerator, RandomSource randomSource, BoundingBox boundingBox, ChunkPos chunkPos, BlockPos pos) {
        ...
        if (templatePosition.getY() >= worldGenLevel.getMinY()) {
    ...
    private int getHeight(BlockPos leftMostCornerPos, BlockGetter level, BlockPos rightMostCornerPos) {
        ...
        for (BlockPos blockpos : BlockPos.betweenClosed(leftMostCornerPos, rightMostCornerPos)) {
            ...
            for (FluidState fluidstate = level.getFluidState(blockpos$mutableblockpos); (blockstate.isAir() || fluidstate.is(FluidTags.WATER) || blockstate.is(BlockTags.ICE)) && y > level.getMinY() + 1; fluidstate = level.getFluidState(blockpos$mutableblockpos)) {
                --y;
    ...
    protected BlockPos getCaveHeight(BlockPos currentStructureCenter, WorldGenLevel level, RandomSource randomSource) {
        ...
        BlockPos.MutableBlockPos chunkCenter = new BlockPos.MutableBlockPos(currentStructureCenter.getX(), level.getMinY() + 3, currentStructureCenter.getZ());
