...
public class DinosaurSpiritEntity extends Entity {
    ...
    private static final EntityDataAccessor<Optional<UUID>> PLAYER_ID = SynchedEntityData.defineId(DinosaurSpiritEntity.class, com.github.alexmodguy.alexscaves.server.entity.ACEntityDataRegistry.OPTIONAL_UUID.get());
    ...
    private void tickTremorsaurus(Player player) {
        ...
        if (target != null) {
            ...
            if (inRange && this.abilityProgress >= 5) {
                if(!dealtDamage && target.hurtOrSimulate(ACDamageTypes.causeSpiritDinosaurDamage(level().registryAccess(), player), 3 + 2 * getEnchantmentLevel())){
                    dealtDamage = true;
                }
                this.setFading(true);
            }
    ...
    protected void readAdditionalSaveData(net.minecraft.world.level.storage.ValueInput tag) {
        if (com.github.alexmodguy.alexscaves.server.misc.NbtCompat.contains(tag, "UsingPlayerUUID")) {
            this.setPlayerUUID(com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getUUID(tag, "UsingPlayerUUID"));
        }
        this.setDinosaurTypeInt(com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getInt(tag, "DinosaurType"));
    }
