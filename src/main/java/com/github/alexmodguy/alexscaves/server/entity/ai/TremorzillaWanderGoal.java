...
public class TremorzillaWanderGoal extends Goal {
    ...
    public BlockPos findWaterBlock(int range) {
        ...
        while (move.getY() < tremorzilla.level().getMaxY() && !tremorzilla.level().getFluidState(move).isEmpty()) {
            move.move(0, 1, 0);
        }
        surfaceY = move.getY();
        ...
        for (int i = 0; i < 15; i++) {
            ...
            if (!tremorzilla.level().getFluidState(blockPos).isEmpty() && !isTargetBlocked(Vec3.atCenterOf(blockPos)) && blockPos.getY() > tremorzilla.level().getMinY() + 1) {
                return blockPos;
            }
        }
        return around;
