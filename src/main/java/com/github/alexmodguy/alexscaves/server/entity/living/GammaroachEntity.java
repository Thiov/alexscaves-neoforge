...
public class GammaroachEntity extends PathfinderMob implements IAnimatedEntity {
    ...
    private static final net.minecraft.world.entity.ai.targeting.TargetingConditions.Selector IRRADIATED_TARGET = (mob, serverLevel) -> {
    ...
    public static boolean canMonsterSpawnInLight(EntityType<GammaroachEntity> entityType, ServerLevelAccessor levelAccessor, EntitySpawnReason mobSpawnType, BlockPos blockPos, RandomSource randomSource) {
    ...
    public static <T extends Mob> boolean checkGammaroachSpawnRules(EntityType<GammaroachEntity> entityType, ServerLevelAccessor iServerWorld, EntitySpawnReason reason, BlockPos pos, RandomSource random) {
    ...
    public void readAdditionalSaveData(net.minecraft.world.level.storage.ValueInput compound) {
        ...
        this.setSprayCooldown(com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getInt(compound, "SprayCooldown"));
    }
    ...
    public void handleEntityEvent(byte b) {
        if (b == 49) {
            ...
            for (int i = 0; i < 8; ++i) {
                ...
                this.level().addParticle(com.github.alexmodguy.alexscaves.client.render.compat.ItemParticleOptionCompat.of(ParticleTypes.ITEM, itemstack), this.getX() + headPos.x, this.getY(0.5) + headPos.y, this.getZ() + headPos.z, (random.nextFloat() - 0.5F) * 0.1F, random.nextFloat() * 0.15F, (random.nextFloat() - 0.5F) * 0.1F);
            }
        } else {
                ...
                if (pickupMonster == null || pickupMonster.isPassengerOfSameVehicle(GammaroachEntity.this)) {
                    ...
                    if (GammaroachEntity.this.distanceTo(target) < 1.5F + target.getBbWidth()) {
                        if (GammaroachEntity.this.getAnimation() == NO_ANIMATION) {
                            ...
                            target.hurtOrSimulate(damageSources().mobAttack(GammaroachEntity.this), (float) GammaroachEntity.this.getAttributeValue(Attributes.ATTACK_DAMAGE));
                        }
                        if (pickupMonster != null) {
                            pickupMonster.stopRiding();
                            pickupMonster = null;
                    ...
                    if (GammaroachEntity.this.distanceTo(pickupMonster) < 1.5F + pickupMonster.getBbWidth()) {
                        ...
                        pickupMonster.hurtOrSimulate(damageSources().cactus(), 1);
                    }
                } else {
                    pickupMonster = null;
                }
