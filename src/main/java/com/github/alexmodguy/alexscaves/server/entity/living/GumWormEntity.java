...
public class GumWormEntity extends Monster implements ICustomCollisions, KaijuMob, ShakesScreen {
    ...
    private static final EntityDataAccessor<Optional<UUID>> RIDING_SEGMENT_UUID = SynchedEntityData.defineId(GumWormEntity.class, com.github.alexmodguy.alexscaves.server.entity.ACEntityDataRegistry.OPTIONAL_UUID.get());
    ...
    public static boolean checkGumWormSpawnRules(EntityType<? extends Monster> entityType, ServerLevelAccessor levelAccessor, EntitySpawnReason mobSpawnType, BlockPos blockPos, RandomSource randomSource) {
    ...
    public void tick() {
        ...
        // Upstream movement chain: bounce up only when hovering over UNDIGGABLE ground (bedrock family);
        // dive back underground when breaching/over air/in fluid. The port's former "stuck detection /
        // return-to-ground" machinery was a band-aid for the inverted canDigBlock above — its
        // heightmap-based Y targets (overworld surface - 10) pointed far above underground caves and
        // caused the head-down spin-in-place churn. Removed; upstream's chain IS the ground-return.
        ...
        } else if ((!isSafeDig(level(), this.blockPosition())) && !this.isLeaping()) {
            if (!canDigBlock(centralStateBelow)) {
        ...
        if(isRidingMode()){
            ...
            if(isTouchingBedrock()){
                ...
                if(surfaceY < worldHeight + 1.0F){
                    upOrDown = 0.8F;
                }else if(this.getY() > level().getMinY() + 3.0F){
                    upOrDown = -0.1F;
                }
    ...
    protected void dropAllDeathLoot(ServerLevel serverLevel, DamageSource damageSource) {
        ...
        if (this.shouldDropLoot((net.minecraft.server.level.ServerLevel) this.level()) && serverLevel.getGameRules().get(net.minecraft.world.level.gamerules.GameRules.MOB_DROPS)) {
            this.dropFromLootTable((net.minecraft.server.level.ServerLevel) level(), damageSource, flag);
            this.dropCustomDeathLoot(serverLevel, damageSource, flag);
        }

        this.dropEquipment((net.minecraft.server.level.ServerLevel) level());
        this.dropExperience((net.minecraft.server.level.ServerLevel) level(), damageSource.getEntity());
    }

    private void dropItemAtSurface(ItemEntity itementity) {
        BlockPos pos = itementity.blockPosition();
        while (level().getBlockState(pos).isSolid() && pos.getY() < level().getMaxY()){
            pos = pos.above();
        }
    ...
    public void spawnDustParticles(boolean surface) {
        if (surfaceY < this.getY(1.0F) && this.getY(1.0F) - surfaceY < 4) {
            ...
            if (!state.isAir()) {
                com.github.alexmodguy.alexscaves.server.entity.util.ParticleCompat.addParticle(level(), new BlockParticleOption(ACParticleRegistry.BIG_BLOCK_DUST.get(), state), true, this.getRandomX(0.8F), surfaceY + random.nextFloat(), this.getRandomZ(0.8F), (random.nextFloat() - 0.5F) * 0.2F, (random.nextFloat() - 0.5F) * 0.2F, (random.nextFloat() - 0.5F) * 0.2F);
            }
        }
    }
    ...
    private double calculateSurfaceY() {
        ...
        while (mutableBlockPos.getY() > level().getMinY() && !level().getBlockState(mutableBlockPos).isSuffocating(level(), mutableBlockPos)) {
            mutableBlockPos.move(0, -1, 0);
        }
    ...
    private void doInitialPosing(LevelAccessor world) {
        ...
        int downCount = 0;
        while (!world.getBlockState(down).isAir()  && downCount < 10 && down.getY() > world.getMinY()) {
            down = down.below();
            downCount++;
        }
    ...
    public void addAdditionalSaveData(net.minecraft.world.level.storage.ValueOutput compoundTag) {
        ...
        if (this.getRidingSegmentUUID() != null) {
            com.github.alexmodguy.alexscaves.server.misc.NbtCompat.putUUID(compoundTag, "RidingSegmentUUID", this.getRidingSegmentUUID());
        }
    ...
    public void readAdditionalSaveData(net.minecraft.world.level.storage.ValueInput compoundTag) {
        ...
        if (com.github.alexmodguy.alexscaves.server.misc.NbtCompat.contains(compoundTag, "GobthumperX") && com.github.alexmodguy.alexscaves.server.misc.NbtCompat.contains(compoundTag, "GobthumperY") && com.github.alexmodguy.alexscaves.server.misc.NbtCompat.contains(compoundTag, "GobthumperZ")) {
            this.setGobthumperPos(new BlockPos(com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getInt(compoundTag, "GobthumperX"), com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getInt(compoundTag, "GobthumperY"), com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getInt(compoundTag, "GobthumperZ")));
        }
        if (com.github.alexmodguy.alexscaves.server.misc.NbtCompat.hasUUID(compoundTag, "RidingSegmentUUID")) {
            this.setRidingSegmentUUID(com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getUUID(compoundTag, "RidingSegmentUUID"));
        }
        this.setTempSummon(com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getBoolean(compoundTag, "TempSummon"));
    }
    ...
    public static boolean canDigBlock(BlockState state) {
        // Upstream formula: AIR IS DIGGABLE. The initial port pasted Corrodent's check here
        // (fluid-empty && canOcclude), which made air "non-diggable" and inverted isSafeDig —
        // a worm in open cave air bounced +0.8/tick to the ceiling and pinned there.
