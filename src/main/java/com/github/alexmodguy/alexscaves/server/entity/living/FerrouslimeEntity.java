...
public class FerrouslimeEntity extends Monster {
    ...
    public void tick() {
        ...
        if (this.entityData.get(ATTACK_TICK) > 0) {
            ...
            if (attackProgress >= 3F && target != null && this.distanceTo(target) < getSlimeSize(1.0F)) {
                target.hurtOrSimulate(damageSources().mobAttack(this), 4F + getHeadCount() * 2);
            }
    ...
    private FerrouslimeEntity makeSlime(int heads, int cooldown) {
        ...
        FerrouslimeEntity ferrouslime = ACEntityRegistry.FERROUSLIME.get().create(level(), net.minecraft.world.entity.EntitySpawnReason.EVENT);
    ...
    public boolean doHurtTarget(net.minecraft.server.level.ServerLevel serverLevel, Entity entityIn) {
        ...
        public void tick() {
            ...
            if (target != null && target.isAlive()) {
                ...
                if (FerrouslimeEntity.this.distanceTo(target) < 1 + FerrouslimeEntity.this.getSlimeSize(1.0F) && FerrouslimeEntity.this.hasLineOfSight(target) && cooldown == 0) {
                    FerrouslimeEntity.this.doHurtTarget((net.minecraft.server.level.ServerLevel) FerrouslimeEntity.this.level(), target);
                    cooldown = 10;
                }
            }
            if (cooldown > 0) {
