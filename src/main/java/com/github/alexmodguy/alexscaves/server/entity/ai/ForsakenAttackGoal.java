...
public class ForsakenAttackGoal extends Goal {
        ...
        if (target != null && target.isAlive()) {
            ...
            if (attemptSonicDamageIn > 0) {
                ...
                if (attemptSonicDamageIn == 0 && this.entity.hasLineOfSight(target)) {
                    target.hurtOrSimulate(ACDamageTypes.causeForsakenSonicBoomDamage(entity.level().registryAccess(), entity), entity.getSonicDamageAgainst(target));
                        ...
                        if (this.entity.getAnimationTick() % 5 == 0) {
                            ...
                            for (LivingEntity living : list) {
                                if (living != this.entity && !this.entity.isAlliedTo(living) && !living.isAlliedTo(entity) && living.distanceTo(entity) <= 14F && !living.getType().builtInRegistryHolder().is(ACTagRegistry.FORSAKEN_IGNORES)) {
                                    living.hurtOrSimulate(ACDamageTypes.causeForsakenSonicBoomDamage(entity.level().registryAccess(), entity), (float) Math.ceil(entity.getSonicDamageAgainst(target) * 0.65F));
                                }
                            }
                        }
                    }
    ...
    private boolean checkAndDealDamage(LivingEntity target, double multiplier, float extraRange) {
        if (entity.hasLineOfSight(target) && entity.distanceTo(target) < entity.getBbWidth() + target.getBbWidth() + extraRange) {
            boolean b = target.hurtOrSimulate(target.damageSources().mobAttack(entity), (float) (multiplier * entity.getAttribute(Attributes.ATTACK_DAMAGE).getValue()));
    ...
    private BlockPos findJumpTarget(LivingEntity target, boolean far) {
        ...
        if (vec3 != null) {
            ...
            if (entity.level().getBlockState(blockpos.below()).isSolidRender() && !entity.level().getBlockState(blockpos).isSolid() && entity.level().isUnobstructed(this.entity, Shapes.create(aabb))) {
                return blockpos;
            }
        }
        return null;
