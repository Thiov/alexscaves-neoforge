    ...
    public static void tick(Level level, BlockPos blockPos, BlockState state, CopperValveBlockEntity entity) {
        ...
        if (entity.movingDown && entity.downProgress < 10.0F) {
            if(entity.downProgress == 0){
                level.playLocalSound((double) blockPos.getX() + 0.5D, (double) blockPos.getY() + 0.5D, (double) blockPos.getZ() + 0.5D, ACSoundRegistry.COPPER_VALVE_CREAK_ON.get(), SoundSource.BLOCKS, 1.0F, level.getRandom().nextFloat() * 0.4F + 0.8F, false);
            }
            entity.downProgress += 0.5F;
        } else if (!entity.movingDown && entity.downProgress > 0.0F) {
            if(entity.downProgress == 10.0F){
                level.playLocalSound((double) blockPos.getX() + 0.5D, (double) blockPos.getY() + 0.5D, (double) blockPos.getZ() + 0.5D, ACSoundRegistry.COPPER_VALVE_CREAK_OFF.get(), SoundSource.BLOCKS, 1.0F, level.getRandom().nextFloat() * 0.4F + 0.8F, false);
            }
            entity.downProgress -= 0.5F;
        }
