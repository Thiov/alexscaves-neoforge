    ...
    protected Vec3 getPosition() {
        ...
        if (start != null && start.isValid() || errorCooldown > 0) {
            ...
            if (!validPieceCenters.isEmpty()) {
                ...
                while(mob.level().getBlockState(randomCenter).isAir() && randomCenter.getY() > mob.level().getMinY()){
                    randomCenter = randomCenter.below();
                }
                while(!mob.level().getBlockState(randomCenter).isAir() && randomCenter.getY() < mob.level().getMaxY()){
