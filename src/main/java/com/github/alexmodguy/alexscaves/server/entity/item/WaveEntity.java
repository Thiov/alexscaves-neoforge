...
public class WaveEntity extends Entity {
    ...
    protected void readAdditionalSaveData(net.minecraft.world.level.storage.ValueInput tag) {
        if (com.github.alexmodguy.alexscaves.server.misc.NbtCompat.hasUUID(tag, "Owner")) {
            this.ownerUUID = com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getUUID(tag, "Owner");
        }
        if (com.github.alexmodguy.alexscaves.server.misc.NbtCompat.contains(tag, "Lifespan")) {
            this.setLifespan(com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getInt(tag, "Lifespan"));
        }
    }
    ...
    private void attackEntities(float scale) {
        ...
        for (LivingEntity entity : this.level().getEntitiesOfClass(LivingEntity.class, bashBox)) {
            if (!isAlliedTo(entity) && !(entity instanceof DeepOneBaseEntity) && (owner == null || !owner.equals(entity) && !owner.isAlliedTo(entity))) {
                entity.hurtOrSimulate(source, scale + 1.0F);
                this.setSlamming(true);
