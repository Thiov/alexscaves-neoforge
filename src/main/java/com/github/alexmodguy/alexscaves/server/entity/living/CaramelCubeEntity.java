...
public class CaramelCubeEntity extends Monster implements PossessedByLicowitch {
    ...
    public static boolean checkCaramelCubeSpawnRules(EntityType<? extends Monster> entityType, ServerLevelAccessor levelAccessor, EntitySpawnReason mobSpawnType, BlockPos blockPos, RandomSource randomSource) {
    ...
    protected void registerGoals() {
        ...
        this.targetSelector.addGoal(1, new NearestAttackableTargetGoal<>(this, Player.class, 10, true, false, (y, serverLevel) -> {
    ...
    public void readAdditionalSaveData(net.minecraft.world.level.storage.ValueInput compound) {
        ...
        this.setSlimeSize(com.github.alexmodguy.alexscaves.server.misc.NbtCompat.getInt(compound, "SlimeSize"), false);
    ...
    private void spawnMeltedCaramel() {
        ...
        for (int j = 0; j < i; ++j) {
            ...
            float f2 = Mth.sin(f) * (float) i * 0.5F * f1;
            float f3 = Mth.cos(f) * (float) i * 0.5F * f1;
            MeltedCaramelEntity meltedCaramel = ACEntityRegistry.MELTED_CARAMEL.get().create(level(), net.minecraft.world.entity.EntitySpawnReason.EVENT);
    ...
    public void remove(Entity.RemovalReason reason) {
        ...
        if (!this.level().isClientSide() && ourSize > 0 && this.isDeadOrDying()) {
            ...
            for (int l = 0; l < slimesSpawned; ++l) {
                ...
                CaramelCubeEntity slime = ACEntityRegistry.CARAMEL_CUBE.get().create(this.level(), net.minecraft.world.entity.EntitySpawnReason.EVENT);
                if (slime != null) {
                    ...
                    com.github.alexmodguy.alexscaves.server.entity.util.EntityCompat.moveTo(slime, this.getX() + (double) f1, this.getY() + 0.5D, this.getZ() + (double) f2, this.random.nextFloat() * 360.0F, 0.0F);
    ...
    protected void dropFromLootTable(DamageSource source, boolean b) {
        if (this.getSlimeSize() == 0) {
            super.dropFromLootTable((net.minecraft.server.level.ServerLevel) level(), source, b);
        }
    }
        ...
        public void tick() {
            ...
            if (livingentity != null) {
                ...
                if(dist < CaramelCubeEntity.this.getBbWidth() + 0.25D + livingentity.getBbWidth() && CaramelCubeEntity.this.hasJumped() && CaramelCubeEntity.this.onGround() && attackLogicCooldown == 0){
                    ...
                    livingentity.hurtOrSimulate(damageSources().mobAttack(CaramelCubeEntity.this), (float) CaramelCubeEntity.this.getAttribute(Attributes.ATTACK_DAMAGE).getValue());
                }
            }
